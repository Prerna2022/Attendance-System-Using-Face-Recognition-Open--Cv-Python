...
def output(filename, sheet,num, name, present):
    my_file = Path('F:\Attendance_Files\\'+filename+str(datetime.now().date())+'.xls')
    if my_file.is_file():
        rb = open_workbook('F:\Attendance_Files\\'+filename+str(datetime.now().date())+'.xls')
        book = copy(rb)
        sh = book.get_sheet(0)
        # file exists
    else:
    ...
    book.save('F:\Attendance_Files\\'+fullname)
    return fullname;
